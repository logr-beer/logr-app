    ...
    @if($beers->count())
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach($beers as $beer)
                @if($selecting)
                    <div
                        wire:click="toggleSelected({{ $beer->id }})"
                        class="cursor-pointer group relative rounded-lg bg-white dark:bg-gray-800 shadow-sm hover:shadow-lg hover:scale-[1.025] transition-all duration-200 {{ in_array($beer->id, $selected) ? 'ring-2 ring-amber-500 ring-offset-2 dark:ring-offset-gray-900' : '' }}"
                    >
                        {{-- Selection circle --}}
                        <div class="absolute top-2 left-2 z-20">
                            @if(in_array($beer->id, $selected))
