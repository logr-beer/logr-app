                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-sm text-amber-700 dark:text-amber-400">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 pl-6 sm:pl-0">
                        @if(in_array('untappd', $missing))
                            <a href="{{ route('admin.api') }}" wire:navigate class="font-medium underline hover:text-amber-800 dark:hover:text-amber-300">Connect Untappd RSS</a>
                            @if(in_array('discord', $missing))
                                <span class="hidden sm:inline">·</span>
                            @endif
                        @endif
                        @if(in_array('discord', $missing))
                            <a href="{{ route('admin.notifications') }}" wire:navigate class="font-medium underline hover:text-amber-800 dark:hover:text-amber-300">Set up Discord notifications</a>
                        @endif
                    </div>
                </div>
                <button wire:click="dismiss" class="shrink-0 p-1 text-amber-400 hover:text-amber-600 dark:hover:text-amber-300 transition-colors">
